...
def _proposal_email(proposal: dict, link: str) -> tuple[str, str, str]:
    ...
    if not isinstance(settings, dict):
        settings = {}
    subject = str(settings.get("subject") or "Your content proposal — Studio 7 Miami").strip()[:200]
    custom_message = str(settings.get("message") or "").strip()
    ...
    intro = (
        ...
        if custom_message
        else f"Hi {client} — here is your Studio 7 Miami content proposal"
        f"{' for ' + html.escape(date_text) if date_text else ''}. "
        "Open it to look it over, then sign and pay when you’re ready."
    )
    body = f"""<!doctype html>
...
<div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">Review your Studio 7 Miami proposal, then sign and pay when ready.</div>
...
<p style="margin:0;font-size:12px;line-height:1.6;"><a href="https://studio7.miami" style="color:#111;text-decoration:none;">studio7.miami</a><span style="color:#C8C8C4;"> · </span><a href="https://book.studio7.miami" style="color:#111;text-decoration:none;">book.studio7.miami</a></p>
    ...
    lead = custom_message or (
        f"Hi {proposal.get('client_name') or 'there'} — here is your Studio 7 Miami content proposal"
        f"{' for ' + date_text if date_text else ''}."
    )
    text = "\n".join(filter(None, [
        lead, "Open it to look it over, then sign and pay when you’re ready.", "",
